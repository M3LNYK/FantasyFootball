...
```{r}
library(tidyverse)
elon <- read_csv("../data/twitterElonMusk.csv")
head(elon)
```
...
- italic type: _this is the first_, and this is the *second award*.
- bold type: __this is the first__, and this is the **second award**.
...
Or perhaps you would like to highlight the highest value from the `hp` column in the description? 
The maximum, i.e., `r max(mtcars$hp)` horsepower, is observed in a car in row `r which.max(mtcars$hp)`.
